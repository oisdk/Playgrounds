...
evalTardisT :: Functor m => TardisT bw fw m a -> (bw, fw) -> m a
evalTardisT t s = fst <$> runTardisT t s
...
execTardisT :: Functor m => TardisT bw fw m a -> (bw, fw) -> m (bw, fw)
execTardisT t s = snd <$> runTardisT t s
...
instance Functor m => Functor (TardisT bw fw m) where
  fmap f xs = TardisT (\s -> (fmap.first) f (runTardisT xs s))
